            ...
            for model_name in dict.fromkeys(self.fallback_models):
                try:
                    response = self.client.chat.completions.create(
                        model=model_name,
                        messages=[
                            {"role": "system", "content": EMAIL_PROMPT + '\n\nAlways respond with valid JSON in this format: {\"greeting\": \"...\", \"introduction\": \"...\"}'},
                    ...
                    intro = EmailIntroduction(
                        greeting=response_json.get("greeting", ""),
                        introduction=response_json.get("introduction", "")
                    )
                    break
                except Exception as e:
                    last_error = e

            if intro is None and last_error:
                raise last_error
